...
//                .cors().configurationSource(createCorsConfigSource()).and()
                .authorizeExchange(
                        exchanges -> exchanges.pathMatchers("/manifest.json",
                                        "/*.png", "/static/**", "/public/**",
                                        "/api/user", "actuator/refresh")
                                .permitAll().anyExchange().authenticated())

                .oauth2Login().and().csrf().disable().oauth2ResourceServer().jwt();
...
//    @Bean
    ...
    public CorsWebFilter corsFilter() {
        return new CorsWebFilter(createCorsConfigSource());
    }
